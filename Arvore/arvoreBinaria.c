...
int insere(arvorebin *raiz, int valor)
{
  ...
  else
  {
    ...
    while (atual != NULL)
    {
      ...
      if (valor > atual->info)
      { // se o valor for maior do que o atual ,ele vai para direita, para obedercer as regras da arvore binaria
        atual = atual->dir;
      }
      else
      { //se nao for ele vai p esqueda
        atual = atual->esq;
      }
    }
    if (valor > ant->info)
...
int remover(arvorebin *raiz, int valor)
{
  ...
  struct NO *atual = *raiz; //o atual recebe a raiz para poder percorrer a arvore toda
  while (atual != NULL)
